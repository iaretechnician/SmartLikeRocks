<hr>
<nav class="main-nav-outer" id="test"><div class="container">
        <ul class="main-nav">

            <li><a href="purekb.php?page=reconfigure">Reconfigure Array</a></li>
            <li><a href="purekb.php?page=portsprotocols">Ports & Protocols</a></li>
            <li><a href="purekb.php?page=pureissues">Errors</a></li>
            <li><a href="purekb.php?page=pureftp">Purity Versions</a></li>
            <li><a href="purekb.php?page=purevpn">Pure VPN</a></li>
            <li><a href="purekb.php?page=purereplication">Replication</a></li>
            <li><a href="purekb.php?page=purepaas">PaaS EG/1</a></li>



            </ul>
        <a class="res-nav_click" href="#"><i class="fa-bars"></i></a>
    </div>
</nav>